...
l1:='SELECT Employees.[Рабочая группа], Employees.ФИО, Employees.[Дата рождения], Employees.Адресс, Employees.[Номер телефона], Groups.[Зона работы] ';
  ...
  Form7.ADOQuery1.SQL.Add(l1+'FROM Groups INNER JOIN Employees ON Groups.[№ Группы] = Employees.[Рабочая группа] WHERE (((Employees.[Рабочая группа])='+ebg+'));');
  Form7.ADOQuery1.Active:=true;
  Form7.Caption:='Сотрудники по группе';
  Form7.ShowModal;
end;
